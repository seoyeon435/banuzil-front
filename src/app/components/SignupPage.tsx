    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-1/2 bg-white flex items-center justify-center px-12 py-12 overflow-y-auto">
        <div className="w-full max-w-[400px]">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-[28px] font-bold text-[#2C1810] mb-2">회원가입</h1>
            <p className="text-[#8C6B5A]">
              이미 계정이 있으신가요?{" "}
              <Link to="/login" className="text-[#FF8C7A] underline hover:text-[#E56B58]">
                로그인
              </Link>
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#8C6B5A]" />
                <input
                  type="text"
            ...
            <div>
              ...
              {password.length > 0 && (
                <div className="flex gap-1 mt-2">
                  <div className={`h-1 flex-1 rounded ${passwordStrength >= 1 ? 'bg-[#E57373]' : 'bg-[#EDD9CC]'}`} />
                  <div className={`h-1 flex-1 rounded ${passwordStrength >= 2 ? 'bg-[#D4956A]' : 'bg-[#EDD9CC]'}`} />
                  <div className={`h-1 flex-1 rounded ${passwordStrength >= 3 ? 'bg-[#6BAF8C]' : 'bg-[#EDD9CC]'}`} />
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                {passwordsMatch && (
                  <Check className="absolute right-12 top-1/2 -translate-y-1/2 w-5 h-5 text-[#6BAF8C]" />
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <select className="w-full h-12 px-4 bg-white border border-[#EDD9CC] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#FF8C7A] focus:border-transparent transition-all text-[#2C1810]">
                <option value="">선택해주세요</option>
                {mbtiTypes.map((type) => (
                  <option key={type} value={type}>{type}</option>
                ))}
              </select>
              <p className="text-xs text-[#8C6B5A] mt-1">나중에 설정해도 괜찮아요</p>
            </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                <div className="relative flex-shrink-0 mt-0.5">
                  <input
                    ...
                    className="appearance-none w-5 h-5 border-2 border-[#EDD9CC] rounded bg-white checked:bg-[#FF8C7A] checked:border-[#FF8C7A] transition-all cursor-pointer"
                  />
                  {termsAccepted && (
            ...
            <Link
              ...
              className={`
                ...
                ${termsAccepted
                  ? 'bg-[#FF8C7A] text-white hover:bg-[#E56B58] shadow-[0_4px_16px_rgba(255,140,122,0.2)] hover:shadow-[0_6px_20px_rgba(255,140,122,0.3)] cursor-pointer'
                  : 'bg-[#EDD9CC] text-[#8C6B5A] cursor-not-allowed opacity-50'
                }
              `}
            >
              다음: 애착 유형 검사
          ...
          <div className="space-y-3">
            ...
            <Link
              to="/signup/attachment-survey"
              className="w-full h-[48px] border-2 border-[#EDD9CC] bg-white text-[#2C1810] rounded-xl hover:bg-[#F5E6D8] transition-all flex items-center justify-center gap-2"
            >
              <span>⬜</span>
              구글로 시작하기
            </Link>
